...
{ 
  ...
  {         
    ...
    {
      printf("Error, not correct PWM pin = %d\r\n", _pin);
    ...
    printf("pin = %d, PWM_CHAN = %d\r\n", _pin, pwm_gpio_to_channel(_pin));
    
    ////////////////////////////////
    
    _enabled = true;
    
    printf("PWM enabled, slice =%d top =%d, div =%d, level =%d\r\n", _slice_num, _PWM_config.top, _PWM_config.div, level);
    ...
    {   
      ...
      if (_frequency != frequency)
      {
        ...
        else
        {
          ...
          printf("Changing PWM frequency to %d and dutyCycle = %d\r\n", frequency, (float) _dutycycle / 1000);
        }
      }
      else if (_enabled)
      {
        if (_dutycycle != dutycycle)
        {
          ...
          printf("Changing PWM DutyCycle to", (float) _dutycycle / 1000, "and keeping frequency =", _frequency);

        }
        else
        {
          printf("No change, same PWM frequency =", frequency, "and dutyCycle =", (float) _dutycycle / 1000);
        }
      }
      ...
      if ( (!_enabled) || newFreq || newDutyCycle )
      {
        ...
        {
          printf("Error, not correct PWM pin = ", _pin);
        ...
        printf("PWM enabled, slice = ", _slice_num, ", _frequency = ", _frequency);
      }
  ...
  inline bool get_enable_disable_status(){
    return _enabled;
  }
  ...
  {           
    ...
    {
      printf("Error, freq must be >=", MIN_PWM_FREQUENCY);
    ...
    printf("_PWM_config.top =", _PWM_config.top, ", _actualFrequency =", _actualFrequency);
